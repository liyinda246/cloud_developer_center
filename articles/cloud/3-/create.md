...
点击持续集成中的构建任务列表，进入应用构建任务详情列表。如下图1-4。
...
点击提取按钮，将在提取文件页签下显示出所提取的配置文件。如下图1-7。

图 1-7
![](images/create/6.png)

可在线编辑所提取的配置文件。如下图1-8。
...
流水线创建的时候，该功能存在于填写流水线配置的时候。如图1-25所示。
...
定时构建策略可以选择一天当中的一个时间，则会在一天中某个时间点进行自动构建。如图1-28所示。
